...
namespace Portable.Text {
	abstract class MonoEncoding : Encoding
	{
		readonly int win_code_page;

		protected MonoEncoding (int codePage) : this (codePage, 0)
		{
		}

		protected MonoEncoding (int codePage, int windowsCodePage) : base (codePage)
		{
	...
	abstract class MonoEncoder : Encoder
	{
		readonly MonoEncoding encoding;

		protected MonoEncoder (MonoEncoding encoding)
		{
		...
		public override int GetByteCount (char[] chars, int index, int count, bool flush)
		{
			...
			unsafe {
				fixed (char* cptr = chars) {
					return GetByteCountImpl (cptr + index, count, flush);
				}
			}
		}
		...
		public override int GetBytes (char[] chars, int charIndex, int charCount, byte[] bytes, int byteIndex, bool flush)
		{
			...
			unsafe {
				fixed (char* cptr = chars) {
					fixed (byte* bptr = bytes) {
						return GetBytesImpl (cptr + charIndex, charCount, bptr + byteIndex, bytes.Length - byteIndex, flush);
					}
				}
			}
		}
